...
# Backward-compat: stop via old pid files if present
[ -f .pid-rust-8001 ] && kill "$(cat .pid-rust-8001)" 2>/dev/null || true
[ -f .pid-proof-gate-8602 ] && kill "$(cat .pid-proof-gate-8602)" 2>/dev/null || true

# Stop via new PID files
[ -f "$RUN_DIR/backend-8001.pid" ] && kill "$(cat "$RUN_DIR/backend-8001.pid")" 2>/dev/null || true
[ -f "$RUN_DIR/proof-gate.pid" ] && kill "$(cat "$RUN_DIR/proof-gate.pid")" 2>/dev/null || true
...
setsid bash -lc 'set -a; [ -f .env ] && source .env || true; set +a; node x402/proof-gate-server.js' > "$LOG_DIR/proof-gate-8602.log" 2>&1 < /dev/null & echo $! > "$RUN_DIR/proof-gate.pid"
...
setsid bash -lc 'set -a; [ -f .env ] && source .env || true; set +a; RUST_LOG=info cargo run' > "$LOG_DIR/backend-8001.log" 2>&1 < /dev/null & echo $! > "$RUN_DIR/backend-8001.pid"
...
echo "[restart] Status"
echo "- proof-gate pid: $(cat "$RUN_DIR/proof-gate.pid" 2>/dev/null || cat .pid-proof-gate-8602 2>/dev/null || echo n/a)"
echo "- rust backend pid: $(cat "$RUN_DIR/backend-8001.pid" 2>/dev/null || cat .pid-rust-8001 2>/dev/null || echo n/a)"

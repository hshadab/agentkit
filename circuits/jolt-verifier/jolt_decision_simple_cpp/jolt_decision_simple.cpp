...
void JOLTDecisionSimple_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void JOLTDecisionSimple_4_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[5] = { 
Num2Bits_0_run,
LessThan_1_run,
LessEqThan_2_run,
GreaterEqThan_3_run,
JOLTDecisionSimple_4_run };
...
ctx->componentMemory[coffset].templateName = "JOLTDecisionSimple";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
int cmp_index_ref_load = -1;
{
std::string new_cmp_name = "confRange";
LessEqThan_2_create(mySignalStart+4,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "confThreshold";
GreaterEqThan_3_create(mySignalStart+19,3+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 3+ctx_index+1;
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 2];
// load src
Fr_sub(&expaux[1],&signalValues[mySignalStart + 0],&circuitConstants[2]); // line circom 22
Fr_mul(&expaux[0],&signalValues[mySignalStart + 0],&expaux[1]); // line circom 22
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[1]); // line circom 23
}}
...
cmp_index_ref_load = 1;
Fr_sub(&expaux[1],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0]); // line circom 38
Fr_mul(&expaux[0],&signalValues[mySignalStart + 0],&expaux[1]); // line circom 38
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 3],&circuitConstants[1]); // line circom 39
}}
...
JOLTDecisionSimple_4_create(1,0,ctx,"main",0);
JOLTDecisionSimple_4_run(0,ctx);
}
